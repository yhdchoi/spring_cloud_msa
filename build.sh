...
docker run -d -p 8086:8086 yhdc/api-gateway:1.0.0

docker build ./image-service -t yhdc/image-service:1.0.0
wait
echo "Image service is containerized. Sart running the image..."
docker run -d -p 8100:8100 yhdc/api-gateway:1.0.0

docker build ./video-catalog-service -t yhdc/avideo-catalog-service:1.0.0
wait
echo "Video catalog service is containerized. Sart running the image..."
docker run -d -p 8101:8101 yhdc/api-gateway:1.0.0
